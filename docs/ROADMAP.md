...
## DISCLAIMER: 
- v7.x.x-LTS version of Jenny will have a shorter support time unlike v5.1.9-LTS. The lack of the package manager access feature will be a negative part of the next long-term support release.
- Another reason for the native package manager access being replaced by Dawn is that the `check-updates` and `upgrade-apps` commands are not even used by `hypernova-developer`, the lonely maintainer.
...
## v11.x.x-LTS [ ]
- Starting from this release, the separated modules from the main `jenny.cpp` file will be permanently deleted from the Jenny Toolkit CLI project.
...
- This will make Jenny a mobile tool that will adapt to any system that it runs on.
